...
AutoCompleteApp::AutoCompleteApp(Trie *t, QWidget *parent)
    : QMainWindow(parent), selectedIndex(-1), isHoveringSuggestion(false) {
  trie = t;
  ...
  connect(searchDelayTimer, &QTimer::timeout, this,
          &AutoCompleteApp::updateSuggestions);
}
...
void AutoCompleteApp::setupUI() {
  ...
  suggestionContainer->setSizePolicy(QSizePolicy::Preferred,
  ...
  connect(inputField, &InputField::navigationKeyPressed, this,
  ...
  connect(prefsAction, &QAction::triggered, [this]() {
    ...
    connect(&dlg, &SettingsDialog::settingsChanged, this,
            &AutoCompleteApp::onSettingsChanged);
    dlg.exec();
  });
...
void AutoCompleteApp::updateInputHeight() {
  ...
  inputField->setMinimumHeight(
      qMin(qMax(60, int(docHeight) + 30), int(this->height() * 0.7)));
}
...
void AutoCompleteApp::selectPrevious() {
  if (suggestionButtons.isEmpty())
    return;
  selectedIndex =
      (selectedIndex - 1 + suggestionButtons.size()) % suggestionButtons.size();
  updateSelection();
}
...
void AutoCompleteApp::updateSelection() {
  for (int i = 0; i < suggestionButtons.size(); i++) {
    ...
    suggestionButtons[i]->setStyleSheet(
        selected ? "background-color:#e1e1e1; color:rgb(27, 27, 27) ; "
                   "border-radius: 10px;"
                 : "background-color:#262626; color: rgba(255, 255, 255, 0.9); "
                   "border-radius: "
                   "10px;");
  }
}
...
void AutoCompleteApp::showSuggestions() {
  if (!suggestionContainer->isVisible()) {
    ...
    connect(fadeAnimation, &QPropertyAnimation::finished, fadeAnimation,
...
void AutoCompleteApp::updateSuggestions() {
  ...
  trie->generateSuggestions(baseWord.toStdString(), maxSuggestions, useBFS,
                            useFreq);

  vector<string> &sugs = trie->suggestionsVector;
  if (!(std::find(sugs.begin(), sugs.end(), baseWord.toStdString()) !=
        sugs.end())) {
  ...
  if (!sugs.empty()) {
    ...
    for (const auto &it : sugs) {
      ...
      if (capitalize) {
        displayText =
            suggestion.left(1).toUpper() + suggestion.mid(1).toLower();
      } else if (allCaps) {
      ...
      connect(btn, &QPushButton::clicked,
              [this, displayText]() { replaceCurrentWord(displayText); });
...
void AutoCompleteApp::closeEvent(QCloseEvent *event) {
  ...
  QPushButton *discardButton =
      msgBox.addButton("Discard", QMessageBox::DestructiveRole);
